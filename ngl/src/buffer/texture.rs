...
impl Texture {
    ...
    pub(crate) fn new(ctx: Rc<Context>, data: &[u8], size: UVec2, params: Parameters) -> Self {
        ...
        unsafe {
            ...
            Self::make(&ctx, nat, data, size, format);
    ...
    pub fn resize(&mut self, data: &[u8], size: UVec2, format: Format) {
        Self::make(&self.ctx, self.nat, data, size, format);
        self.size = size;
    }
    ...
    pub(crate) fn bind(&self, unit: Sampler2d) {
        unsafe {
            ...
            self.ctx.bind_texture(glow::TEXTURE_2D, Some(self.nat));
        }
    }

    fn make(ctx: &Context, nat: NativeTexture, data: &[u8], size: UVec2, format: Format) {
        ...
        unsafe {
            ctx.bind_texture(glow::TEXTURE_2D, Some(nat));
            ctx.tex_image_2d(
                glow::TEXTURE_2D,
                0,
                format.gl() as _,
